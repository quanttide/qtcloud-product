...
面向 QtCloud Studio 开发者的指南，覆盖架构设计、交互设计与产品需求三个板块。
...
- [架构设计](architecture.md)：领域模型类名（UserActivity / UserTask / UserStory / StoryMap）与 UI 组件类名（ActivityLane / TaskCard / StoryCard / StoryMapCanvas）的命名映射、数据流向与组合逻辑。
