...
- **精简约定**：卡片只展示用户故事标题（用户语言，动词开头），不做其他信息展示
- 描述、发布阶段、状态等数据保留在领域模型中（CLI 加工、其他视图使用），不在卡片上渲染
- 白底 + 细浅边框（`grey[300]`）+ 圆角 6 + 极淡阴影（视觉减负）
...
| 故事卡片 | `#FFFFFF` 底 / `grey 300` 边框 | 故事标题 12.5px，黑色 87% |
| 分隔线 | `grey` 0.5px | Release 行头 |

### 8.2 状态信息

状态（🟢 完成 / 🟡 进行中 / ⚪ 待办）与发布阶段保留在领域模型中，卡片上不再展示（卡片只展示标题）。

### 8.3 视觉减负约定

- 故事卡片白底细边框，不按阶段着色、不展示标签
