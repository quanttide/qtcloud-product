# Release Line 设计更新说明

**更新日期**: 2026年1月7日  
**变更类型**: 核心设计重构

---

## 📋 变更概述

将用户故事地图的分类维度从 **优先级（Priority）** 改为 **发布阶段（ReleasePhase）**。

### 之前的设计 ❌
```
优先级维度：Must Have / Should Have / Could Have
├─ 左侧标签栏：显示 MUST/SHOULD/COULD 三层
└─ 故事卡片：按优先级着色
```

### 新的设计 ✅
```
发布阶段维度：MVP 版本 / 未来迭代
├─ 可拖动的细浅色虚线（Release Line）
├─ 上方（MVP）：白色卡片 + 状态圆点
└─ 下方（Future）：白色卡片 + 状态圆点
```

---

## 🔧 技术改动

### 1. 数据模型 (`story_map_models.dart`)

**删除**：
```dart
enum StoryPriority {
  must('Must Have', 0),
  should('Should Have', 1),
  could('Could Have', 2);
}
```

**新增**：
```dart
enum ReleasePhase {
  mvp('MVP 版本'),
  future('未来迭代');
}
```

**更新 UserStory 类**：
```dart
class UserStory {
  // 之前
  final StoryPriority priority;
  
  // 现在
  final ReleasePhase phase;  // MVP 或 Future
}
```

### 2. UI 组件 (`story_card.dart`)

**颜色方案更新**：
- MVP（绿色）：`#E8F5E9` / `#27AE60`
- Future（蓝色）：`#EBF5FB` / `#3498DB`

**标签显示**：
```dart
// 之前
_buildTag(story.priority.label)  // "Must Have"

// 现在
_buildTag(story.phase.label)     // "MVP 版本"
```

### 3. 画布组件 (`story_map_canvas.dart`)

**移除**：左侧的 MUST/SHOULD/COULD 标签栏

**新增**：可拖动的 Release Line
```dart
// Release Line（发布线）
- 细浅色虚线（#90A4AE），视觉弱化，仅表示上下阶段切分
- 可垂直拖动调整位置
- 上方显示 "MVP" 标签（浅红小胶囊）
- 下方显示 "Future" 标签（浅灰小胶囊）
- 支持实时拖拽反馈（使用 GestureDetector）
```

**交互流程**：
1. 用户按住 Release Line 附近（±10px）
2. 垂直拖动调整线的位置
3. 实时更新 `mvpLinePosition`（0.0 ~ 1.0）
4. 触发 `onMVPLineMove` 回调

### 4. 种子数据 (`data/seed_data.dart`)

**数据分布**（以 qtcloud-product 为例）：

| 用户活动 | 任务 | MVP | Future | 状态 |
|----------|------|-----|--------|------|
| 管理用户故事 | 维护故事地图 | 2 | 0 | Done |
| 管理用户故事 | 细化用户故事 | 2 | 0 | InProgress |
| 制定版本计划 | 划定 MVP 边界 | 2 | 0 | InProgress |
| 制定版本计划 | 对齐版本节奏 | 0 | 1 | To Do |
| 管理产品组合 | 登记产品目录 | 0 | 2 | To Do |
| 管理产品组合 | 查看组合视图 | 0 | 1 | To Do |
| 管理产品组合 | 记录决策 | 0 | 1 | To Do |

**Release Line 位置**：`mvpLinePosition: 0.4`（界于「制定版本计划」和「管理产品组合」之间）

> 注：样本数据已迁移为种子数据 `data/seed_data.dart`，包含三个实验产品（qtcloud-devops / qtcloud-product / qtcloud-code）。

---

## 🎨 视觉变化

### 新的布局结构

```
┌─────────────────────────────────────────┐
│  AppBar: "qtcloud-product"              │
├─────────────────────────────────────────┤
│                                          │
│  [管理用户故事] [制定版本计划] [管理产品组合] │ MVP 版本
│  ████         ████            ----       │
│                                          │
│ ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  │ ← Release Line（可拖动）
│                                          │
│  [管理用户故事] [制定版本计划] [管理产品组合] │ 未来迭代
│  ----         ----            ████       │
│                                          │
└─────────────────────────────────────────┘
```

### 故事卡片样式（视觉减负）

- **卡片**：白色底 + 细浅边框，不按阶段着色
- **状态圆点**：🟢 完成 / 🟡 进行中 / ⚪ 待办（卡片右上角小圆点）
- **发布阶段**：极小灰色文字（"MVP 版本" / "未来迭代"）弱化提示

---

## 🚀 使用示例

### 基础使用

```dart
StoryMapCanvasPage(
  mapData: storyMap,
  onStoryMove: (story, newTaskId) {
    // 处理故事移动
  },
  onStoryTap: (story) {
    // 处理故事点击
  },
  onMVPLineMove: (position) {
    // 处理 Release Line 拖动
    // position: 0.0 ~ 1.0（相对于画布高度）
    print('MVP Line 移动到: ${position * 100}%');
  },
)
```

### 动态更新 Release Line

```dart
// 在外部改变 Release Line 位置
final newStoryMap = storyMap.copyWith(
  mvpLinePosition: 0.6,  // 移到 60% 处
);
```

---

## ✅ 编译状态

- ✅ 零编译错误
- ✅ 所有 Dart 文件通过静态检查
- ✅ 可直接运行

---

## 🔄 向后兼容性

**破坏性变更**：
- `StoryPriority` 已删除，改为 `ReleasePhase`
- 所有使用 `priority` 的代码需改为 `phase`
- 旧版数据格式需要迁移

**迁移指南**：
```dart
// 旧数据
UserStory(..., priority: StoryPriority.must)

// 新数据
UserStory(..., phase: ReleasePhase.mvp)
```

---

## 🎯 下一步

- [ ] 实现背景网格线（CustomPaint）
- [ ] 支持Release Line 拖拽后的数据持久化
- [ ] 添加动画过渡效果
- [ ] 实现故事卡片随 Release Line 自动分类

---

**状态**: ✅ MVP 实现完成  
**最后更新**: 2026年1月7日
